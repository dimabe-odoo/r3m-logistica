...
class HrContract(models.Model):
    ...
    apv_id = fields.Many2one('custom.benefits.rrhh', 'Institución APV',
                             domain=lambda self: [('data_type_id', 'in', [self.get_apv_data(), self.get_safe_data()])])
    ...
    supplementary_insurance_id = fields.Many2one('custom.benefits.rrhh', string='Seguro Complementario',
                                                 domain=lambda self: [('data_type_id', '=', self.get_safe_data())])
    ...
    permanent_discounts_ids = fields.One2many('custom.permanent_discounts', 'contract_id',
                                              string='Descuentos Fijos/Permanentes')
    ...
    first_progressive_vacation_date = fields.Date('Fecha inicio Vacación Progresiva',
                                                  help="Fecha de inicio vacación progresiva")

    custom_document_ids = fields.One2many('custom.contract.document', 'contract_id', 'Documentos Asociados')

    is_contract_work = fields.Boolean('Contracto por obra')
    ...
    def onchange_section(self):
        if self.section_id.name:
            if self.section_id.code == 'D':
                ...
                section_amount = \
                self.env['custom.indicators.data'].search([('name', '=', self.section_id.name + ' - Monto')],
                                                          order='id desc')[0]
        ...
        if self.wage and self.section_type_id:
            ...
            for section in sections:
                if section.code == 'D':
                    ...
                    max_salary_section = \
                    self.env['custom.indicators.data'].search([('name', '=', section.name + ' - Tope')],
                                                              order='id desc')[0]
